import { z } from 'zod';
import { REGEX_PATTERNS, TRANSACTION_LIMITS, SECURITY_CONFIG } from '../constants';
...
export const createTransactionSchema = z.object({
  ...
  amount: z.number()
    ...
    .min(TRANSACTION_LIMITS.MIN_AMOUNT, `Minimum amount is $${TRANSACTION_LIMITS.MIN_AMOUNT}`)
    .max(TRANSACTION_LIMITS.MAX_AMOUNT, `Maximum amount is $${TRANSACTION_LIMITS.MAX_AMOUNT}`),
...
export const createWireTransferSchema = z.object({
  ...
  swiftCode: z.string().regex(REGEX_PATTERNS.SWIFT_CODE, 'Invalid SWIFT code').optional(),
...
export const createKYCDocumentSchema = z.object({
  ...
  fileSize: z.number().positive('File size must be positive').max(5 * 1024 * 1024, 'File size cannot exceed 5MB'),
...
export const transactionQuerySchema = paginationSchema.extend({
  ...
  type: z.enum(['DEPOSIT', 'WITHDRAWAL', 'TRANSFER', 'PAYMENT', 'FEE']).optional(),
