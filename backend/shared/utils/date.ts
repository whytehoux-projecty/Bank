import { format, addDays, subDays, startOfDay, endOfDay, isValid, parseISO } from 'date-fns';
 ...
 */
export const getDateRange = (period: 'today' | 'yesterday' | 'last7days' | 'last30days' | 'thisMonth' | 'lastMonth'): { start: Date; end: Date } => {
  const now = new Date();
 ...
 */
export const isLegalAge = (dateOfBirth: Date | string, minAge: number = 18): boolean => {
 ...
 */
export const utcToLocal = (utcDate: Date | string, timezone: string = 'America/New_York'): Date => {
