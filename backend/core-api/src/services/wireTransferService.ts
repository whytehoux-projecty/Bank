...
export class WireTransferService {
  ...
  static async createWireTransfer(
    ...
    // Pre-flight balance check for fast early feedback (non-authoritative)
    ...
    // Pre-flight daily limit check (non-authoritative)
    ...
    const result = await prisma.$transaction(async (tx: any) => {
      // Atomically deduct balance only if sufficient funds remain.
      // This prevents race conditions where two concurrent requests both pass
      // the pre-flight check but would together overdraw the account.
      const deducted = await tx.$executeRaw`
        UPDATE "Account"
        SET balance = balance - ${totalAmount}, "lastTransactionAt" = NOW()
        WHERE id = ${senderAccountId} AND balance >= ${totalAmount}
      `;
