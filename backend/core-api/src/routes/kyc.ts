...
/**
 * Enforce that the :userId path param matches the authenticated user.
 * Admins (present in AdminUser table) may access any user's KYC data.
 */
const requireKycOwnership = async (request: FastifyRequest, reply: FastifyReply) => {
  const authUserId = (request as any).user?.userId;
  const { userId } = request.params as { userId: string };

  if (authUserId === userId) return; // owner — always allowed

  // Check if caller is an admin
  const admin = await prisma.adminUser.findUnique({
    where: { email: (request as any).user?.email },
    select: { id: true, status: true },
  });

  if (admin && admin.status === 'ACTIVE') return; // admin — allowed

  return reply.status(HTTP_STATUS.FORBIDDEN).send({
    success: false,
    error: ERROR_CODES.INSUFFICIENT_PERMISSIONS,
    message: 'Access denied. You can only access your own KYC documents.',
  });
};
...
export default async function kycRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/:userId/documents', {
    preHandler: [fastify.authenticate, requireKycOwnership],
