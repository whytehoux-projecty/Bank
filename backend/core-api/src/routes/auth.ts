...
export default async function authRoutes(fastify: FastifyInstance) {
  ...
  // GET /api/auth/profile — alias used by the e-banking portal
  fastify.get(
    '/profile',
    {
      preHandler: [fastify.authenticate],
      schema: { description: 'Get current user profile', tags: ['Authentication'], security: [{ bearerAuth: [] }] },
    },
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        const user = request.user as any;
        const userProfile = await prisma.user.findUnique({
          ...
          select: {
            ...
            phone: true, status: true, kycStatus: true,
            createdAt: true, lastLoginAt: true,
          },
        });
        if (!userProfile) return reply.status(404).send({ error: 'User Not Found', message: 'User profile not found' });
        reply.send({ user: userProfile });
      } catch (error) {
        fastify.log.error('Get profile error:', error);
        reply.status(500).send({ error: 'Profile Fetch Failed', message: 'Failed to fetch user profile' });
      }
    }
  );
  ...
  fastify.put(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        const user = request.user as any;
        const { firstName, lastName, phone, preferredLanguage, preferredCurrency, notificationPreferences } =
          request.body as any;
        ...
        reply.status(500).send({ error: 'Profile Update Failed', message: 'Failed to update user profile' });
      }
    }
  );
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        if (!dbUser) return reply.status(404).send({ error: 'User Not Found', message: 'User not found' });
        ...
        reply.status(500).send({ error: 'Password Change Failed', message: 'Failed to change password' });
      }
    }
  );
