...
export default async function twoFactorRoutes(fastify: FastifyInstance) {
    ...
    // Disable 2FA — verified with the current TOTP code (6-digit authenticator token)
    fastify.post(
        '/disable',
        {
            preHandler: [fastify.authenticate],
            schema: {
                description: 'Disable Two-Factor Authentication using current TOTP code',
                tags: ['2FA'],
                security: [{ bearerAuth: [] }],
                body: {
                    type: 'object',
        ...
        async (request: FastifyRequest, reply: FastifyReply) => {
            try {
                ...
                // Verify the TOTP code before disabling — proves the user controls the authenticator
                const isValid = await twoFactorAuthService.verifyToken(user.userId, code);
                ...
                // Disable by passing verified flag; service clears the secret
