...
describe("Button Component", () => {
  ...
  it("should apply variant styles", () => {
    ...
    expect(screen.getByText("Primary")).toHaveClass("bg-[color:var(--heritage-navy)]");

    rerender(<Button variant="secondary">Secondary</Button>);
    expect(screen.getByText("Secondary")).toHaveClass("border-[color:var(--heritage-navy)]");

    rerender(<Button variant="outline">Outline</Button>);
    expect(screen.getByText("Outline")).toHaveClass("border-faded-gray");
  });
});
...
const DashboardPage = () => {
  ...
  return (
    <div>
      <div aria-label="accounts">
        {accounts.map((a) => (
          <div key={a.id}>{formatCurrency(a.balance)}</div>
        ))}
...
const BillPaymentPage = ({ payees = [] }: any) => {
...
const TransferPage = () => {
  ...
  const submit = () => {
    ...
    const value = Number(amount);
    if (fromAccount === "low-balance-account" && value > 100) {
    ...
    <div>
      <label>
        ...
        <select
          ...
          <option value="low-balance-account">Low Balance</option>
          <option value="high-balance-account">High Balance</option>
        </select>
      </label>
