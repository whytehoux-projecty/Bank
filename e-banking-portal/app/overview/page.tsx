    ...
    try {
      ...
      if (profileResult.status === "fulfilled") {
        ...
        setUserPreferences((prev) => ({
          ...prev,
          language: profileUser.preferredLanguage || prev.language,
          currency: profileUser.preferredCurrency || prev.currency,
        }));
      } else {
  ...
  const processChartData = (txs: any[]) => {
    ...
    const hour = new Date().getHours();
        ...
        if (hour < 12) {
            ...
            if (lang === 'de') return 'Guten Morgen';
            return 'Good morning';
        }
        if (hour < 18) {
            if (lang === 'es') return 'Buenas tardes';
            if (lang === 'fr') return 'Bon après-midi';
            if (lang === 'de') return 'Guten Tag';
            return 'Good afternoon';
        }
        if (lang === 'es') return 'Buenas noches';
        if (lang === 'fr') return 'Bonsoir';
        if (lang === 'de') return 'Guten Abend';
        return 'Good evening';
  };
      ...
      <Tabs
        ...
        <TabsList>
          <TabsTrigger value="overview">
            {translate(userPreferences.language, "nav.overview")}
          </TabsTrigger>
          <TabsTrigger value="analytics">
            {translate(userPreferences.language, "nav.analytics")}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="overview" className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <Card className="animate-fade-in-up animate-delay-100 hover-lift">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium font-sans">
                  {translate(userPreferences.language, "overview.totalBalance")}
                </CardTitle>
            ...
            <Card className="animate-fade-in-up animate-delay-200 hover-lift">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium font-sans">
                  {translate(userPreferences.language, "overview.incomeMonth")}
                </CardTitle>
            ...
            <Card className="animate-fade-in-up animate-delay-300 hover-lift">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium font-sans">
                  {translate(userPreferences.language, "overview.expensesMonth")}
                </CardTitle>
            ...
            <Card className="animate-fade-in-up animate-delay-400 hover-lift">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium font-sans">
                  {translate(userPreferences.language, "overview.savingsGoals")}
                </CardTitle>
              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground text-right">
                    {((stats.totalBalance / stats.savingsGoal) * 100).toFixed(
                      1,
                    )}
                    % of goal
                  </p>
